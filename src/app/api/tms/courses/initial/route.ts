...
// POST /api/tms/courses/initial - Tạo course với đầy đủ 5 steps
const createInitialCourse = async (body: unknown, request: NextRequest) => {
  ...
  const result = await db.$transaction(async (tx) => {
    ...
    if (courseData.syllabus && courseData.syllabus.length > 0) {
      await tx.course_syllabus.createMany({
        data: courseData.syllabus.map((week) => ({
          course_id: nextId,
          week: week.week,
          topic: week.topic || '',
          ...
          attachments: week.attachments ? JSON.stringify(week.attachments) : null
        }))
      });
    }

    // 3. Create course prerequisites entries if provided
    if (courseData.prerequisites && courseData.prerequisites.length > 0) {
      // For now, we'll store them as text in the prerequisites field
      // Later we can enhance this to create proper prerequisite relationships
      // when we have a course lookup system
    }
...
export const POST = withErrorHandling(
  async (request: NextRequest) => {
    ...
    return await createInitialCourse(body, request);
  },
